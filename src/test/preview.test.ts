...
suite('openPreview', () => {
	...
	test('keeps the zoom of each preview independent', async () => {
		const zoomed = await vscode.workspace.openTextDocument({
			content: '- Activity A',
		});
		...
		edit.insert(
			zoomed.uri,
			zoomed.positionAt(zoomed.getText().length),
			'\n- Activity B',
		);
		edit.insert(
			other.uri,
			other.positionAt(other.getText().length),
			'\n- Activity X',
		);
		await vscode.workspace.applyEdit(edit);
	...
	test('does not update the webview when another document changes', async () => {
		const previewed = await vscode.workspace.openTextDocument({
			content: '- Activity A',
		});
		const other = await vscode.workspace.openTextDocument({
			content: '- Another Activity',
	...
	test('loads the bundled webview script', async () => {
		...
		assert.ok(
			/<script src="[^"]*webview\.js"><\/script>/.test(panel.webview.html),
		);
		panel.dispose();
	});
